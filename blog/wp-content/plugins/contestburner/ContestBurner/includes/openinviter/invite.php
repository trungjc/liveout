...
function ers($ers)
	...
	if (!empty($ers))
		{
		$contents="<table cellspacing='0' cellpadding='0' style='border:1px solid red;' align='center'><tr><td valign='middle' style='color:red;padding:5px;'>";
	...
	elseif ($step=='send_invites')
		...
		if (empty($_POST['provider_box'])) $ers['provider']='Provider missing !';
			...
			if ($internal) $ers['internal']=$internal;
				...
				if (empty($_POST['subject_box'])) $ers['subject_box']='Subject missing !';
				else $_POST['subject_box']=strip_tags($_POST['subject_box']);
				...
				$message=array('subject'=>$_POST['subject_box'],'body'=>stripslashes($_POST['message_box']),'attachment'=>"");
		...
		if (count($ers)==0)
			...
			if ($sendMessage===-1)
				{
				$message_footer="\r\n\r\nThis invite was sent from ".$_POST['email_box']." via ContestBurner - http://www.contestburner.com";
...
$contents.="<form action='' id='openinviter_form' method='POST' name='openinviter' onSubmit='return false;'>".ers($ers).oks($oks);
if (!$done)
	{
	if ($step=='get_contacts')
		{
		$contents.="<table align='center' class='thTable' cellspacing='2' cellpadding='0' style='border:none;'>
			<tr class='thTableRow'><td align='right'><label for='email_box'>Email</label></td><td><input id='email_box' class='thTextbox' type='text' name='email_box' value='{$_POST['email_box']}'></td></tr>
			<tr class='thTableRow'><td align='right'><label for='password_box'>Password</label></td><td><input id='password_box' class='thTextbox' type='password' name='password_box' value='{$_POST['password_box']}'></td></tr>
			<tr class='thTableRow'><td align='right'><label for='provider_box'>Email provider</label></td><td><select id='provider_box' class='thSelect' name='provider_box'><option value=''></option>";
		...
		$contents.="</select></td></tr>
			<tr class='thTableImportantRow'><td colspan='2' align='center'><input id='oi_import_submit' class='thButton' type='submit' name='import' value='Import Contacts'></td></tr>
		...
		$contents.="<table class='thTable' cellspacing='0' cellpadding='0' style='border:none;'>
				<tr class='thTableRow'>
					<td align='right' valign='top' style='white-space:nowrap;'><label for='subject_box'>Email Subject: </label></td>
					<td><input type='text' id='subject_box' name='subject_box' style='width:300px;' value='{$_POST['subject_box']}' /></td>
				</tr>
				<tr class='thTableRow'>
					<td align='right' valign='top' style='white-space:nowrap;'><label for='message_box'>Email Message: </label></td>
					<td><textarea rows='5' cols='50' id='message_box' name='message_box' class='thTextArea' style='width:300px;'>{$_POST['message_box']}</textarea></td>
				</tr>
				<tr class='thTableRow'><td align='center' colspan='2'><input id='oi_send_submit' type='submit' name='send' value='Send Invites' class='thButton' ></td></tr>
			</table>";
	}
