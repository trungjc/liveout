...
*/
	...
	define('CBSQL_VERSION','2.11');
	define('CBOBJECT','CBOBJECT',true);
	define('CBOBJECT_K','CBOBJECT_K',true);
	define('CBARRAY_A','CBARRAY_A',true);
	define('CBARRAY_N','CBARRAY_N',true);
	...
	*/
	...
	class cbdb
	{
		...
		*/
		...
		function get_row($query=null,$output=CBOBJECT,$y=0)
		{
			...
			elseif ( $output == CBARRAY_A )
			{
				return $this->last_result[$y]?get_object_vars($this->last_result[$y]):null;
			}
			// If the output is an numerical array then return row as such..
			elseif ( $output == CBARRAY_N )
			{
			...
			{
				$this->print_error(" \$db->get_row(string query, output type, int offset) -- Output type must be one of: CBOBJECT, CBARRAY_A, CBARRAY_N");
			}
		...
		*/
		...
		function get_results($query=null, $output = CBOBJECT)
		{
			...
			} elseif ( $output == CBOBJECT_K ) {
				if ( $this->last_result ) {
					foreach ( $this->last_result as $row ) {
						$row_array = get_object_vars( $row );
						$first_column_value = array_shift( $row_array );
						if ( !isset( $new_array[ $first_column_value ] ) ) {
							$new_array[ $first_column_value ] = $row;
						}
					}
					return $new_array;
				} else {
			...
			} elseif ( $output == CBARRAY_A || $output == CBARRAY_N )
			{
				if ( $this->last_result )
				{
					$i=0;
		...
		*/
		...
		function vardump($mixed='')
		{
			...
			{
				echo "<font color=800080><b>ezSQL</b> (v".CBSQL_VERSION.") <b>Variable Dump..</b></font>\n\n";
			}
		...
		*/
		...
		function debug($print_to_screen=true)
		{
			...
			{
				echo "<font color=800080 face=arial size=2><b>ezSQL</b> (v".CBSQL_VERSION.") <b>Debug..</b></font><p>\n";
			}
			...
			if ( $this->last_result )
			{
				...
				$i=0;
				foreach ( $this->get_results(null,CBARRAY_N) as $one_row )
				{
					$i++;
