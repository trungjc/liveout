  ...
  $.fn.smartAutoComplete = function(){    
    ...
    $.event.special.showResults = {
      ...
      _default: function(ev){    
        ...
        //type ahead
        if(options.typeAhead && (raw_results[0].name.toLowerCase().substr(0, $(context).val().length) == $(context).val().toLowerCase()) ){
          var suggestion = raw_results[0].name; //options.typeAheadExtractor($(context).val(), raw_results[0]); 
